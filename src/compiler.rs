...
fn depth(e: &Expr) -> i32 {
    match e {
        ...
        Expr::Let(binds, expr) => {
            let bind_depth = binds.iter().enumerate().map(|x| depth(&x.1.1) + x.0 as i32).max().unwrap_or(0);
            bind_depth + depth(expr) + binds.len() as i32
        },
        Expr::Id(_) => 0,
        ...
        Expr::Call(_, es) => es.iter().enumerate().map(|x| depth(&x.1) + x.0 as i32).max().unwrap_or(0),
    }
}

pub fn compile_definitions(defs: &im::HashMap<String, Definition>, label: &mut i32) -> Vec<Instr> {
    let mut instrs: Vec<Instr> = Vec::new();
    for (_, def) in defs.iter() {
        let depth = depth(&def.body);
        let offset: i64 = (depth * 8).into();
        let mut func_env: im::HashMap<String, i32> = im::HashMap::new();
        for (i, arg) in def.params.iter().enumerate() {
            func_env = func_env.update(arg.clone(), (depth + (i as i32 + 1)) * 8);
        }
        instrs.append(&mut vec![
            Instr::ILabel(def.name.clone()),
            Instr::ISub(Val::Reg(Reg::RSP), Val::Imm(round_up(offset))),
        ]);
        instrs.append(&mut compile_main(&def.body, 0, &func_env, label, "", defs));
        instrs.append(&mut vec![
            Instr::IAdd(Val::Imm(round_up(offset)), Val::Reg(Reg::RSP)),
            Instr::IRet(),
        ]);
    }
    instrs
